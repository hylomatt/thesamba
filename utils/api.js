...
import { parseHome, parseClassifieds, parseForums, parseForum, parseTopic, parseGallery, parseCommunity, parseTechnical, parseArchives, parseAbout } from './parsers'
...
export const getHome = async (path) => {
  return await getPage(path).then((r) => {
    return parseHome(path, r)
  })
}

export const getClassifieds = async (path) => {
  return await getPage(path).then((r) => {
    return parseClassifieds(path, r)
  })
}

export const getForums = async (path) => {
  return await getPage(path).then((r) => {
    return parseForums(path, r)
  })
}

export const getForum = async (path) => {
  return await getPage(path).then((r) => {
    return parseForum(path, r)
  })
}

export const getTopic = async (path) => {
  return await getPage(path).then((r) => {
    return parseTopic(path, r)
  })
}

export const getGallery = async (path) => {
  return await getPage(path).then((r) => {
    return parseGallery(path, r)
  })
}

export const getCommunity = async (path) => {
  return await getPage(path).then((r) => {
    return parseCommunity(path, r)
  })
}

export const getTechnical = async (path) => {
  return await getPage(path).then((r) => {
    return parseTechnical(path, r)
  })
}

export const getArchives = async (path) => {
  return await getPage(path).then((r) => {
    return parseArchives(path, r)
  })
}

export const getAbout = async (path) => {
  return await getPage(path).then((r) => {
    return parseAbout(path, r)
  })
}
